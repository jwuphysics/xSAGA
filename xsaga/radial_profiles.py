...
def compute_interloper_cdf(redshifts, radial_bins, interloper_surface_density=2.34):
    ...
    angular_surface_area = np.array(
        [np.pi * (radial_bins * deg2kpc) ** 2 for deg2kpc in deg_per_kpc]
    )
...
def compute_nonsatellite_cdf(N_unrelated_lowz, radial_bins):
    ...
    surface_area = np.pi * radial_bins ** 2
...
def bootstrap_saga_cdf(radial_bins, M_r_lim=M_r_lim, N_boot=100):
    ...
    SAGA_AT_36KPC = boot_cdf_saga.min(1, keepdims=True)
